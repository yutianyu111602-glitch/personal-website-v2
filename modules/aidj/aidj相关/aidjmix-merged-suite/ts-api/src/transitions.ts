...
export function planTransitions(seq:TrackFeature[], techno=true, simpleHeadTail=false):TransitionPlan{
  const items:PlaylistItem[]=[]; for(let i=0;i<seq.length;i++){ const cur=seq[i], nxt=seq[i+1];
    ...
    const it:PlaylistItem={ track:cur, startAt: cur.cueInSec ?? 0, endAt: cur.cueOutSec ?? cur.durationSec, crossfadeBeats: cross,
      automation: nxt? (simpleHeadTail? [] : [{type:'filter',mode:'hipass_ramp',from_hz:80,to_hz:260,duration_beats:cross}]): [] };
    items.push(it); }
