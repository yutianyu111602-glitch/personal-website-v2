// glsl-generators.template.glsl — 生成器/位移/蒙版算法合集（移植版）
...
vec2 curl(vec2 uv, float scale){
  ...
  float cx = ny0 - ny1; float cy = nx1 - nx0;
  return normalize(vec2(cx, -cy));
}

float worley(vec2 uv, float scale){
  uv *= scale; vec2 i = floor(uv); vec2 f = fract(uv); float d = 1.0;
  for(int y=-1;y<=1;y++){
    for(int x=-1;x<=1;x++){
      vec2 g = vec2(float(x), float(y));
      vec2 o = vec2(N21(i + g), N21(i + g + 19.0));
      vec2 p = g + o - f; d = min(d, dot(p,p));
    }
  }
...
float lic(vec2 uv, vec2 dir, float len){
  vec2 px = 1.0/uResolution; float acc=0.0, w=0.0;
  for(int i=-8;i<=8;i++){
    float t = float(i)/8.0; vec2 offs = dir * t * len * px; float n = snoise( (uv + offs)*6.0 ); float ww = 1.0 - abs(t);
    acc += n*ww; w+=ww;
  }
...
vec4 rdStep(vec2 uv, float F, float K){
  vec2 px = 1.0/uResolution; vec2 off[8];
  off[0]=vec2( 1, 0); off[1]=vec2(-1,0); off[2]=vec2(0, 1); off[3]=vec2(0,-1);
  off[4]=vec2( 1, 1); off[5]=vec2(-1,1); off[6]=vec2(1,-1); off[7]=vec2(-1,-1);
  float a = texture2D(uStateA, uv).r; float b = texture2D(uStateA, uv).g; float lapA=0.0, lapB=0.0;
  ...
  float Da=1.0, Db=0.5; float ra = Da*lapA - a*b*b + F*(1.0 - a); float rb = Db*lapB + a*b*b - (K+F)*b;
  a += ra*1.0; b += rb*1.0; return vec4(clamp(a,0.0,1.0), clamp(b,0.0,1.0), 0.0, 1.0);
}
