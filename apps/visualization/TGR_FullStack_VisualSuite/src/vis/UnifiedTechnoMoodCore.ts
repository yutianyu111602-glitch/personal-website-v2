...
function pickMicroMods(now:number, af:AudioFeatures, perfHigh:boolean){
  const mods: MicroMod[] = [];
  const can = (id:string, cd=600)=>{ (pickMicroMods as any)._cool||(pickMicroMods as any)._cool={}; const t=(pickMicroMods as any)._cool[id]||0; if(t<=now){ (pickMicroMods as any)._cool[id]=now+cd; return true } return false };
